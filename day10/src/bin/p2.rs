...
fn part1(input: &str) -> usize {
    ...
    for y in 0..maze.board.len() {
        let mut north = false;
        let mut south = false;
        for x in 0..maze.board[y].len() {
            if path.path.contains(&(y, x)) {
                match maze.board[y][x] {
                    Element::VerticalPipe => {
                        south = !south;
                        north = !north;
                    }
                    Element::NorthEastPipe | Element::NorthWestPipe => north = !north,
                    Element::SouthEastPipe | Element::SouthWestPipe => south = !south,
                    _ => continue,
                }
            } else {
                if north && south {
